...
5 rubric-pagina's + `/reflectie` als `.mdx` bestanden in `app/<route>/page.mdx`. `/bronnen` is een `page.tsx` die alle data **inline** definieert (drie arrays: `interviews`, `opdrachten`, `apaSources`) — géén import uit `content/bronnen.ts`. De `interviews`-array bevat ook focusgroep en observaties-kaarten. Daaronder hangen detailpagina's:
- Opdrachten: `app/bronnen/opdracht-N/` — aanwezige nummers: 1–11, 13, 16–17, 19–23, 30
- Interviews: `app/bronnen/interview-1/`, `interview-2/` (samen 2 interviewpagina's met 3 middelbare scholieren)
